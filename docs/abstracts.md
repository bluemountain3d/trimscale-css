...
@include mx.fontSetup(
  $font: 'primary', // font role, see fontRoles in trimscale.config.ts
...
@include mx.generate-color-tokens($tokens: $brand-tokens, $default-scheme: dark);
...
| `mx.and-down($breakpoint)`      | Below the *next* breakpoint after the given one (string keys only)        |
...
`$breakpoint` accepts either a key from the `$breakpoints` map (`'tablet'`, `'laptop'`, ...) or a raw number (treated as px if unitless). `and-down` and `only` need a string key, since they look up the *next* entry in the map, there's no "next" for an arbitrary number.
