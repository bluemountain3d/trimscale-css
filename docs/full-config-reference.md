...
This page has to be kept in sync by hand when the config shape changes, it's not generated from the types.
...
| ------------------ | ---------------------------- | :------: | ------------------------------------------------------------------------------------------------- |
| `fonts`           | `Record<string, FontSource>` |    Yes    | Font sources keyed by family name. `FontSource` is `{ source: 'local' \| 'cdn' \| 'manual', ... }`, shape depends on `source`. |
...
| `fallbackDefault` | `FontFallbacks`              |    Yes    | Fallback stack used when a family has no `fallback` of its own. One of `'sans-serif' \| 'serif' \| 'monospace' \| 'system-ui' \| 'cursive'`. |
...
| `minTypeScale`, `maxTypeScale` | `TypeScaleNames \| TypeScaleValues` |    Yes    | Modular-scale ratio at each end, a name (`'Minor Third'`) or its numeric value (`1.2`). |
| `precision`                  | `1 \| 2 \| 3 \| 4 \| 5 \| 6`        |    Yes    | Decimal places in generated clamp() values.  |
...
| -------------------------------------- | -------------------------------------------- | :------: | -------------- | ------------------------------------------------------------------------ |
| `approach`                          | `'coupled' \| 'independent'`                 |    Yes    | Both          | Picks the growth model, see the guide for the full comparison.       |
...
| `macroRangeMax`                     | `4\|5\|6\|7\|8\|10\|12\|14\|16`               |    No    | `independent`  | Ceiling (px) of `--unit-macro`'s clamp. Default `8`.                 |
...
| Property         | Type              | Required | Description                                                             |
| ------------------ | ----------------- | :------: | --------------------------------------------------------------------------- |
| `defaultScheme`  | `'light' \| 'dark'` |    Yes    | Which scheme backs the static hex fallback tier for browsers without `oklch()`/`light-dark()` support. |
...
## Custom color token maps (`{name}ColorTokens`)

Optional. Any number of extra palettes alongside `baseColorTokens`, same `ColorTokensMap` shape as above, keyed as `{name}ColorTokens` (the shipped example config includes `campaignColorTokens`). Referenced in `semanticColorAliases` via `tokenMap`.

→ Full guide: [design-tokens.md#color-tokens](design-tokens.md#color-tokens)

## `semanticColorAliases`

Optional. Semantic names (e.g. `'text-muted'`) aliasing a token from `baseColorTokens` or a custom tokenMap.

| Property     | Type                                          | Required | Description                                                            |
| -------------- | ----------------------------------------------- | :------: | -------------------------------------------------------------------------- |
| `token`      | `string`                                      |    Yes    | Token name to alias, looked up in `tokenMap`.                          |
| `tokenMap`   | `'baseColorTokens' \| \`${string}ColorTokens\`` |    No    | Which map `token` lives in. Default `'baseColorTokens'`.               |
| `opacity`    | `number`                                      |    No    | Opacity (0–1) applied to all oklch/hex variants.                       |
| `lightness`  | `number \| { light: number, dark: number }`    |    No    | Multiplier (not an absolute value), single or per-mode.                |
| `chroma`     | `number \| { light: number, dark: number }`    |    No    | Multiplier (not an absolute value), single or per-mode.                |

→ Full guide: [design-tokens.md#color-tokens](design-tokens.md#color-tokens) · derivation mechanics: [abstracts.md](abstracts.md#fnget-color-tokentoken-tokens-opacity-lightness-chroma)
