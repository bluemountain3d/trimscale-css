...
| Token          | Description                                                                                                                                                                                                                                                                                                        |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `--vwx`        | Adaptive viewport unit. `1vw` normally; switches to `2vh` on ultrawide screens (≥ 21:9 ratio + ≥ 944 px height, `$ultrawide-height-threshold-px`) to prevent runaway scaling                                                                                                                                       |
...
| `--unit-macro` | Base spacing unit for large steps, an independent fluid clamp (`getFluidClamp($base-grid-size, macroRangeMax)`), rounded to the nearest pixel. Scales `$base-grid-size` (default `4`) up to `spacingSetup.macroRangeMax` (default `8`, must be ≥ `4`) across `fluidScale`'s viewport range (`minWidth`–`maxWidth`) |
...
The token _names_ below (`surface-base`, `accent`, `text-muted`, etc.) come from `baseColorTokens`, `campaignColorTokens` (or any other `{name}ColorTokens` map you add), and `semanticColorAliases` in `trimscale.config.ts`. The starter palette shipped in the default config is meant to be replaced with your own values, not treated as fixed brand colors.

Each `baseColorTokens`/`campaignColorTokens` entry is a hand-picked `{ oklch, hex }` pair per light/dark mode, there's no build step deriving them from a single source color. [oklch.com](https://oklch.com/) is a convenient picker for choosing the OKLCH values and reading off the matching hex.

`semanticColorAliases` entries don't need their own hand-picked pair: instead of duplicating oklch math for a near-duplicate color, they reference an existing token by name and can adjust its `$opacity`/`$lightness`/`$chroma`, via the `get-color-token()` function (see [abstracts.md](abstracts.md)) under the hood.
